    ...
    switch ($action) {
      ...
      case 'update':
		...
		if (FURIKANA_NESESSARY) {
	        if (strlen($customers_firstname_kana) < ENTRY_FIRST_NAME_MIN_LENGTH) {
	          $error = true;
	        ...
	        if (strlen($customers_lastname_kana) < ENTRY_LAST_NAME_MIN_LENGTH) {
	          $error = true;
...
?>
  ...
  if (entry_telephone == "" || entry_telephone.length < <?php echo ENTRY_TELEPHONE_MIN_LENGTH; ?>) {
...
<?php
  if ($error == true) {
    if ($entry_telephone_error == true) {
      echo zen_draw_input_field('entry_telephone', $cInfo->entry_telephone, zen_set_field_length(TABLE_ADDRESS_BOOK, 'entry_telephone', 15)) . '&nbsp;' . ENTRY_TELEPHONE_NUMBER_ERROR;
    } else {
       echo $cInfo->entry_telephone . zen_draw_hidden_field('entry_telephone');
    }
  } else {
    echo zen_draw_input_field('entry_telephone', $cInfo->entry_telephone, zen_set_field_length(TABLE_ADDRESS_BOOK, 'entry_telephone', 15), true);
  }
?></td>
          </tr>
          <tr>
...
<?php
  if ($processed == true) {
    echo $cInfo->entry_fax . zen_draw_hidden_field('entry_fax');
     ...
     echo zen_draw_input_field('entry_fax', $cInfo->entry_fax, zen_set_field_length(TABLE_ADDRESS_BOOK, 'entry_fax', 15));
  }
?></td>
          </tr>
        </table></td>
...
//      $search = "where c.customers_lastname like '%" . $keywords . "%' or c.customers_firstname like '%" . $keywords . "%' or c.customers_email_address like '%" . $keywords . "%'";
    ...
    $new_fields=', a.entry_telephone, a.entry_company, a.entry_street_address, a.entry_city, a.entry_postcode, c.customers_authorization';
