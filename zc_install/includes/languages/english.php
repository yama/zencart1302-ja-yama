 ...
 */
  ...
  define('NAVI_PHPBB_SETUP', 'phpBB Setup');
...
You may also have given the wrong database name (<strong>Does it exist?</strong> <strong>Did you create it?</strong> -- NOTE: Zen Cart&trade; does not create a database for you.).<br /><br />Please review all of the entries and ensure that they are correct.');
  ...
  case ('69'):
    ...
    define('POPUP_ERROR_TEXT', 'Zen Cart doesn\'t operate if the setting of register_globals is not set Off because register_globals has the possibility of causing a serious problem on security.(from v1.3.0.2-l10n-jp-3)<br/>The setting of register_globals cannot be changed with .htaccess in the server when this message is displayed.<br/>There are two solution to solve this problem.<br/><br/><br/>1. Change the setting of the server.<br/>2. Reinstall Zen Cart to the server which is able to be set register_globals Off.');
  break;
  case ('70'):
...
&nbsp;&nbsp; / or \\ or . <br /><br />Please select a different prefix. We recommend something simple like "zen_" .');
  ...
  break;
  case ('90'):
    define('POPUP_ERROR_HEADING','Zen Cart v1.3.0.2-l10n-jp-3 supports MySQL 5.0.x');
    define('POPUP_ERROR_TEXT','If you do encounter SQL errors while using Zen Cart&trade; on MySQL 5, please post them to our support forum (after searching to see if the message has already been reported).');
  break;
  case ('91'):
  ...
  case ('95'):
    ...
    define('POPUP_ERROR_TEXT', 'In a part of rental server,  there is difference of directory to store contents between SSL and non-SSL in displaying contents. If it is such as environment, you also have to install Zen Cart in the directory for SSL as well as non-SSL. The physiscal HTTPS path is the path to the directory where your Zen Cart files are installed for SSL. If you do not need SSL or your environment do not need for you to install Zen Cart for SSL directory, this item can be blank.');
  break;
}
