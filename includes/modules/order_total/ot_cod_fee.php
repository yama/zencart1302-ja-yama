 ...
 * @copyright Portions Copyright (c) 2002 Thomas PlgOkers http://www.oscommerce.at
 ...
 */
  ...
  class ot_cod_fee {
    ...
    function process() {
	  ...
	  if (substr_count($_SESSION['shipping']['id'], 'sagawa') !=0) $cod_zones = split("[:,]", MODULE_ORDER_TOTAL_COD_FEE_SAGAWA);
	  if (substr_count($_SESSION['shipping']['id'], 'yamato') !=0) $cod_zones = split("[:,]", MODULE_ORDER_TOTAL_COD_FEE_YAMATO);
...
//lagt tilk servicepakke her!!!!
    function keys() {
      return array('MODULE_ORDER_TOTAL_COD_STATUS',
      			   ...
      			   'MODULE_ORDER_TOTAL_COD_FEE_PERWEIGHTUNIT',
      			   ...
      			   'MODULE_ORDER_TOTAL_COD_FEE_NITTSU',
      			   'MODULE_ORDER_TOTAL_COD_TAX_CLASS');
    }

    function install() {
      global $db;
      $db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, set_function, date_added) values ('インストール状態', 'MODULE_ORDER_TOTAL_COD_STATUS', 'true', '', '6', '1','zen_cfg_select_option(array(\'true\'), ', now())");
